...
| <a name="app-proxy-buffers"></a>routable application | service | [router.deis.io/proxyBuffers](#app-proxy-buffers) | `8 4k|8k` | nginx `proxy_buffers` settings expressed in number and size of the buffers in bytes (no suffix), kilobytes (suffixes `k` and `K`), or megabytes (suffixes `m` and `M`). |
| <a name="app-proxy-buffer-size"></a>routable application | service | [router.deis.io/proxyBufferSize](#app-proxy-buffer-size) | `4k|8k` | nginx `proxy_buffer_size` setting expressed in bytes (no suffix), kilobytes (suffixes `k` and `K`), or megabytes (suffixes `m` and `M`). |
| <a name="app-proxy-busy-buffers-size"></a>routable application | service | [router.deis.io/proxyBusyBuffersSize](#app-proxy--busy-buffers-size) | `8k|16k` | nginx `proxy_busy_buffers_size` setting expressed in bytes (no suffix), kilobytes (suffixes `k` and `K`), or megabytes (suffixes `m` and `M`). |
